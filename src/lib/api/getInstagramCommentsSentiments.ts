// lib/api/getInstagramCommentsSentiments.ts
...
type GetInstagramCommentsSentimentsResponse = {
...
export async function getInstagramCommentsSentiments(competitorId: string): Promise<GetInstagramCommentsSentimentsResponse> {
    console.log('🚀 getInstagramCommentsSentiments iniciada con competitor_id:', competitorId);
    ...
    try {
        ...
        const url = `/api/socialAnalysis/instagram/commentsAnalysis/getInstagramCommentsSentiments?competitor_id=${encodeURIComponent(competitorId.trim())}`;
            ...
            try {
                ...
                if (errorDetails && typeof errorDetails === 'object') {
                    ...
                    if (response.status === 404) {
                        errorMessage = `No se encontraron comentarios con análisis de sentimientos para el competitor ID "${competitorId}" o el competitor ID no existe`;
                    }

                    // Manejo específico para error 401
                    if (response.status === 401) {
                        errorMessage = `No tienes autorización para acceder al análisis de sentimientos de comentarios de Instagram de este competidor. ${errorMessage}`;
                    }
        ...
        const data: GetInstagramCommentsSentimentsResponse = await response.json();
        console.log('✅ Datos parseados exitosamente:', {
            competitorId: competitorId,
            totalComments: data.total_comments || 0,
            commentsAnalyzed: data.results?.length || 0,
            sentimentDistribution: data.results?.reduce((acc, comment) => {
        ...
        if (invalidScores.length > 0) {
            ...
            throw new Error(`Se encontraron ${invalidScores.length} comentarios con scores de sentimiento inválidos`);
        }
        ...
        if (commentsWithoutScores.length > 0) {
            ...
            throw new Error(`Se encontraron ${commentsWithoutScores.length} comentarios sin scores de sentimiento`);
        }

        // Validación de que top_label sea uno de los labels válidos
        const validLabels = ['POS', 'NEG', 'NEU'];
        ...
        if (invalidTopLabels.length > 0) {
            ...
            throw new Error(`Se encontraron ${invalidTopLabels.length} comentarios con etiquetas de sentimiento inválidas`);
        }
        ...
        console.log('✅ getInstagramCommentsSentiments completada exitosamente');
        return data;
        
    } catch (error) {
        console.error('💥 Error en getInstagramCommentsSentiments:', error);
        
        if (error instanceof Error) {
            if (error.name === 'AbortError') {
                throw new Error('La petición se agotó el tiempo de espera. Por favor, intenta de nuevo.');
            }
            throw new Error(error.message || 'Ocurrió un error desconocido al obtener el análisis de sentimientos de comentarios de Instagram.');
        } else {
