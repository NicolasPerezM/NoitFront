...
export function GET({ request, params }: Parameters<APIRoute>[0]) {
  return (async () => {
    try {
      console.log('🔍 Iniciando petición a Instagram topic analysis...');
      ...
      const apiUrl = `https://noit.com.co/api/v1/analyze-competitors/instagram-comments/topic-analysis/${competitorId}`;
      ...
      console.log('📊 Total de temas analizados:', Object.keys(data || {}).length);
      
      // Verificar que la respuesta tenga la estructura esperada (basada en el JSON proporcionado)
      if (!data || typeof data !== 'object') {
        ...
        return new Response(
          JSON.stringify({ 
            ...
            receivedData: data,
            expectedStructure: 'Se esperaba un objeto con temas (Tema_1, Tema_2, etc.)'
          }), 
          { 
            status: 500,
            headers: { 'Content-Type': 'application/json' }
      ...
      console.log('✅ Análisis de temas de comentarios de Instagram obtenido exitosamente');
      
      // Log adicional para debug de temas
      const topicSummary = Object.entries(data).map(([topicKey, topicData]: [string, any]) => ({
        tema: topicKey,
        comentarios: topicData.cantidad_comentarios,
        palabras_principales: Object.keys(topicData.palabras_con_pesos || {}).slice(0, 3)
      }));
      console.log('📈 Resumen de temas:', topicSummary);
      
      // Calcular estadísticas adicionales
      const totalComments = Object.values(data).reduce((sum: number, topic: any) => 
        sum + (topic.cantidad_comentarios || 0), 0
      );
      console.log('💬 Total de comentarios analizados:', totalComments);
