...
export function GET({ request, params }: Parameters<APIRoute>[0]) {
  return (async () => {
    try {
      console.log('🔍 Iniciando petición a Instagram comment emotions...');
      ...
      // Hacer fetch al endpoint externo con el competitor_id para análisis de emociones
      const apiUrl = `https://noit.com.co/api/v1/analyze-competitors/instagram-comments/emotion-analysis/${competitorId}`;
      ...
      console.log('📊 Total de comentarios analizados:', data?.total_comments || 0);
      console.log('💬 Comentarios con emociones encontrados:', data?.results?.length || 0);
      ...
      console.log('✅ Análisis de emociones de comentarios de Instagram obtenido exitosamente');
      
      // Log adicional para debug de emociones
      const emotionCounts = data.results.reduce((acc: Record<string, number>, comment: any) => {
        const emotion = comment.top_label;
        acc[emotion] = (acc[emotion] || 0) + 1;
        return acc;
      }, {});
      console.log('😊 Distribución de emociones:', emotionCounts);
